...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateMainBox(cityData) {
    ...
    mainBox.innerHTML = `
            <img src="./assets/images/icon/${
              cityData.icon
            }" alt="Weather Icon" style="width: 80px; height: 80px; margin-bottom: 24px;"><br>
  ...
  function updateWeather() {
    morningEl.textContent = `${formatTemperature(forecastData.morning.tempC)}`;
    afternoonEl.textContent = `${formatTemperature(
      forecastData.afternoon.tempC
    )}`;
    eveningEl.textContent = `${formatTemperature(forecastData.evening.tempC)}`;
  }
  updateWeather();
});
